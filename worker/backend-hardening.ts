...
type User = { id: string; username: string; display_name: string; role: Role };
...
async function auth(request: Request, env: PosEnv): Promise<User | null> {
  ...
  return env.DB.prepare(`SELECT u.id,u.username,u.display_name,u.role FROM sessions s JOIN users u ON u.id=s.user_id WHERE s.token_hash=? AND s.expires_at>CURRENT_TIMESTAMP AND u.active=1`).bind(await digest(token)).first<User>();
}
...
async function guardedLogin(request: Request, env: PosEnv, url: URL) {
  ...
  if (response.status === 401) {
    ...
    await env.DB.prepare(`DELETE FROM login_throttle WHERE identifier=?`).bind(identifier).run();
  }
  return response;
}
...
async function saveUser(request: Request, env: PosEnv, current: User, userId?: string) {
  ...
  if (userId) {
    ...
    const existing = await env.DB.prepare(`SELECT role FROM users WHERE id=?`).bind(userId).first<{ role: Role }>();
    ...
    if (hash) await env.DB.prepare(`UPDATE users SET display_name=?,username=?,role=?,active=?,password_hash=?,updated_at=CURRENT_TIMESTAMP WHERE id=?`).bind(name,username,role,data.active?1:0,hash,uid).run();
    else await env.DB.prepare(`UPDATE users SET display_name=?,username=?,role=?,active=?,updated_at=CURRENT_TIMESTAMP WHERE id=?`).bind(name,username,role,data.active?1:0,uid).run();
  } else {
    if (!data.password || data.password.length < 8) return problem(env, request, 400, "Password must be at least 8 characters.");
    await env.DB.prepare(`INSERT INTO users(id,username,password_hash,display_name,role,active) VALUES(?,?,?,?,?,?)`).bind(uid,username,await passwordHash(data.password),name,role,data.active?1:0).run();
  }
  return json(env, request, { id: uid }, userId ? 200 : 201);
}
...
async function factoryReset(request: Request, env: PosEnv, current: User) {
  ...
  await env.DB.batch([
    env.DB.prepare(`DELETE FROM service_payments`),
    env.DB.prepare(`DELETE FROM service_check_items`),
    env.DB.prepare(`DELETE FROM service_checks`),
    env.DB.prepare(`DELETE FROM order_item_addons`),
    env.DB.prepare(`DELETE FROM payments`),
    env.DB.prepare(`DELETE FROM inventory_transactions`),
    ...
    env.DB.prepare(`DELETE FROM shifts`),
    env.DB.prepare(`DELETE FROM audit_log`),
    env.DB.prepare(`DELETE FROM login_throttle`),
    env.DB.prepare(`UPDATE inventory_items SET quantity_base=0,updated_at=CURRENT_TIMESTAMP`),
    env.DB.prepare(`UPDATE restaurant_tables SET current_guests=0,status='available',updated_at=CURRENT_TIMESTAMP`),
  ]);
