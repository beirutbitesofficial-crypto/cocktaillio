...
async function bootstrap(request: Request, env: PosEnv, user: AuthUser) {
  const [menu, categories, addons, inventory, tables, reservations, expenses, users, shift, metrics, recentOrders] = await Promise.all([
    ...
    env.DB.prepare(`SELECT id,name,sort_order FROM menu_categories WHERE active=1 ORDER BY sort_order,name`).all(),
...
async function createCategory(request:Request,env:PosEnv,user:AuthUser){if(!can(user,["admin","manager"]))return problem(env,request,403,"Manager permission required.");const data=await body<{name?:string}>(request);const name=data.name?.trim().replace(/\s+/g,' ');if(!name||name.length<2||name.length>50)return problem(env,request,400,"Category name must be 2–50 characters.");const categoryId=id('category');const order=await env.DB.prepare(`SELECT COALESCE(MAX(sort_order),0)+10 value FROM menu_categories`).first<{value:number}>();await env.DB.prepare(`INSERT INTO menu_categories(id,name,sort_order,active) VALUES(?,?,?,1)`).bind(categoryId,name,order?.value??10).run();return json(env,request,{id:categoryId,name,sortOrder:order?.value??10},201);}
async function deleteCategory(request:Request,env:PosEnv,user:AuthUser,categoryId:string){if(!can(user,["admin","manager"]))return problem(env,request,403,"Manager permission required.");const used=await env.DB.prepare(`SELECT COUNT(*) count FROM menu_items WHERE category_id=?`).bind(categoryId).first<{count:number}>();if(used?.count)return problem(env,request,409,"Move or delete this category's menu items first.");await env.DB.prepare(`UPDATE menu_categories SET active=0,updated_at=CURRENT_TIMESTAMP WHERE id=?`).bind(categoryId).run();return json(env,request,{id:categoryId,deleted:true});}
...
export async function handlePosApi(request: Request, env: PosEnv, url: URL): Promise<Response | null> {
  ...
  try {
    ...
    if(url.pathname==='/api/categories'&&request.method==='POST')return createCategory(request,env,user);const categoryMatch=url.pathname.match(/^\/api\/categories\/([^/]+)$/);if(categoryMatch&&request.method==='DELETE')return deleteCategory(request,env,user,decodeURIComponent(categoryMatch[1]));
