...
export default function CatalogPage(){
  return <main className="catalog">
    <header><div><Link href="/">← Dashboard</Link><h1>Menu</h1><p>{posMenu.length} items · {posCategories.length} categories · no images</p></div><div className="quick"><Link href="/service">New Order</Link><Link href="/bar">Bar</Link><Link href="/kitchen">Kitchen</Link></div></header>
    {posCategories.map(cat=>{
      const items=posMenu.filter(i=>i.category===cat.name);
